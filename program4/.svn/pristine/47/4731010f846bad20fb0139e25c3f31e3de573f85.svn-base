...
#define printusers() printf("Program by FRYJC\n");
...
public:
	Program4()
	{
		...
		{			
			...
			App->Display();
			handleEvents();
			render.setModelTransform(translateFromInput*rotationSpinStep*rotationFromInput*translateToOrigin);
		}
	}
...
private:
	...
	glm::mat4 rotationFromInput = glm::mat4(1);
	glm::mat4 rotationSpinStep;
	glm::mat4 currentRotate = glm::mat4(1);
	glm::mat4 spinrotate = glm::mat4(1);
	void updateRotate(glm::ivec2 & oldPos, glm::ivec2 & newPos)
	{
		...
		currentRotate = glm::rotate(glm::mat4(1.0f), phi, axis);
		if (oldPos != newPos){
			spinrotate = currentRotate;
		}
		rotationFromInput =currentRotate * rotationFromInput;
	...
	void updateXYTranslate(glm::ivec2 & oldPos, glm::ivec2 & newPos)
	{
		...
		//
		//
		glm::vec3 translationVect = glm::vec3(newPos - oldPos, 0)*XY_SENSITIVITY;
		translationVect.y = -translationVect.y;
		translateFromInput *= glm::translate(glm::mat4(1), translationVect);
	...
	void updateZTranslate(glm::ivec2 & oldPos, glm::ivec2 & newPos)
	{
		...
		//
		//
		glm::vec3 translationVect = glm::vec3(0,0,sqrt(pow(newPos.x,2)+pow(newPos.y,2))-sqrt(pow(oldPos.x,2)+pow(oldPos.y,2)))*Z_SENSITIVITY;
		translateFromInput *= glm::translate(glm::mat4(1), translationVect);
	}
	...
	void updateAspectRatio(int const & newWidth, int const & newHeight)
	{
		...
		//
		//
		glm::mat4 P = glm::scale(glm::perspective(60.0f, (float)RESOLUTION / (float)RESOLUTION, 0.1f, 100.0f),glm::vec3((float)newHeight/newWidth,1.0f,1.0f));
		render.setProjectionTransform(P);
	}
	...
	void spinCube()
	{
		...
		//
		if (spinning)
			rotationSpinStep = spinrotate * rotationSpinStep;
	}
